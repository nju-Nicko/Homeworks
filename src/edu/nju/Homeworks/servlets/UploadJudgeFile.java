...
/**
 * Servlet implementation class UploadJudgeFile
 */
@WebServlet("/UploadJudgeFile")
public class UploadJudgeFile extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UploadJudgeFile() {
        super();
			...
			for(FileItem item: list){
				if(item.isFormField()){   //判断FileItem是一个文件上传对象还是普通表单对象
					...
					split=info.split(" ");
					 ...
					 fd.updateAssistantJudgeFile(new FileBean(split[2], Integer.parseInt(split[0]), ub.getName(), filename));
				}
			}
